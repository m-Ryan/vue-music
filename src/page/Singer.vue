  ...
  <div>
    <Header />
          ...
          <div class="singer-info float-left">
            ...
            <ul class="mod-data">
                <li class="mod-data-item">
                  <router-link :to="'/song/list/'+data.id">单曲<strong>{{data.artist.musicSize}}</strong></router-link>
                </li>
                <li class="mod-data-item">
                  <router-link :to="'/song/list/'+data.id">专辑<strong>{{data.artist.albumSize}}</strong></router-link>
                </li>
                <li class="mod-data-item">
                  <router-link :to="'/song/list/'+data.id">MV<strong>{{data.artist.mvSize}}</strong></router-link>
                </li>
            </ul>
            <div class="text-left"><span class="play_more_btn" @click="toPlay">播放全部单曲</span></div>
          </div>
        </div>
      </div>
      <div class="songlist main">
        ...
        <div v-if="data">
           <ul v-show="index<10 || showMore" :class="(index%2==0?'even ':'odd ') +' songlist_item clearfix'" v-for="(item, index) in data.songlist" :key="index">
            ...
            <div class="text-center"><el-button class="max-width" type="success" @click="getMore">{{showMore?"收起":"显示更多"}}</el-button></div>
        </div>
      </div>
